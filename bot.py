...
# ============================================================
# 2. ПАРСИНГ ПОДАРКОВ СО СТРАНИЦЫ t.me/username
# ============================================================
async def parse_gifts(username: str):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=10) as resp:
                ...
                gifts = []
                # Ищем ссылки на подарки (обычно в виде /nft/...)
                for link in soup.find_all('a', href=True):
                    href = link.get('href')
                    if href and '/nft/' in href:
...
async def check_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    gifts = await parse_gifts(username)
    ...
    if gifts:
        report += f"🎁 *Подарки ({len(gifts)}):*\n"
        report += " · ".join(gifts[:10]) + "\n\n"
    else:
        report += "🎁 *Подарки:* не найдены или скрыты\n\n"
