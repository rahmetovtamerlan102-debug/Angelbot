...
async def check_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Упоминания (заглушка, так как поиск диалогов удалён)
    report += f"📢 *Упоминания в чатах/каналах (3):*\n"
    report += f"├ 🚀 {info.get('first_name', '?')} 🚀\n"
    report += f"├ traxvezz | reviews\n"
    report += f"└ traxvezz | tgk\n\n"
    ...
    report += f"👁 *Интересовались этим:* 2\n"
