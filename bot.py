...
async def webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Отправляем данные в Mini App через ответ
        response_data = info.copy()
        response_data["scam"] = scam_results

        # Обновляем интерфейс через WebApp
        await update.message.reply_text(
            f"Данные для @{username} получены. Откройте Mini App снова, чтобы увидеть обновлённый интерфейс.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔍 Обновить Spectra", web_app=WebAppInfo(url=f"https://{os.environ.get('RENDER_EXTERNAL_HOSTNAME')}"))]
            ])
        )
